...
class VendorsWindow:
    ...
    def load_vendors(self):
        ...
        for vendor in vendors:
            self.tree.insert('', tk.END, values=(
                ...
                vendor['address'] or '',
                vendor['pib'] or '',
                vendor['registration_number'] or '',
                vendor['bank_account'] or ''
    ...
    def delete_vendor(self):
        ...
        if messagebox.askyesno("Potvrda", "Da li ste sigurni da želite da obrišete ovog dobavljača?"):
            vendor_id = self.tree.item(selection[0])['tags'][0]
            if self.db.delete_vendor(vendor_id):
                messagebox.showinfo("Uspeh", "Dobavljač je uspešno obrisan.")
                self.load_vendors()
            else:
                messagebox.showerror("Greška", "Ne možete obrisati dobavljača koji ima račune.")
...
class VendorDialog:
    ...
    def load_vendor_data(self):
        ...
        if vendor:
            self.vendor_name_entry.insert(0, vendor['vendor_name'])
            self.address_entry.insert(0, vendor['address'] or '')
            self.pib_entry.insert(0, vendor['pib'] or '')
            self.reg_entry.insert(0, vendor['registration_number'] or '')
            self.bank_entry.insert(0, vendor['bank_account'] or '')
    ...
    def save(self):
        ...
        try:
            if self.vendor_id:
                self.db.update_vendor(self.vendor_id, vendor_name, address, pib, reg_number, bank_account)
                messagebox.showinfo("Uspeh", "Dobavljač je uspešno izmenjen.")
            else:
                self.db.add_vendor(vendor_name, address, pib, reg_number, bank_account)
